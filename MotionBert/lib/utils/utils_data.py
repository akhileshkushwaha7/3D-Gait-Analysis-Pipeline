...
def crop_scale(motion, scale_range=None):
    '''
        Motion: [(M), T, 17, 3].
        If scale_range is None, return original coordinates without scaling or normalization.
        Otherwise, normalize to [-1, 1].
    '''
    result = copy.deepcopy(motion)
    if scale_range is None:
        return result  # Return original coordinates (e.g., pixel coordinates for 2D)
    valid_coords = motion[motion[..., 2]!=0][:,:2]
    if len(valid_coords) < 4:
        return np.zeros(motion.shape)
    xmin = min(valid_coords[:,0])
...
def crop_scale_3d(motion, scale_range=None):
    '''
        Motion: [T, 17, 3]. (x, y, z)
        If scale_range is None, return original coordinates without scaling or normalization.
        Z is relative to the first frame's root.
    '''
    result = copy.deepcopy(motion)
    result[:,:,2] = result[:,:,2] - result[0,0,2]  # Root-relative Z
    if scale_range is None:
        return result  # Return original coordinates
    xmin = np.min(motion[...,0])
    xmax = np.max(motion[...,0])
    ymin = np.min(motion[...,1])
    ymax = np.max(motion[...,1])
