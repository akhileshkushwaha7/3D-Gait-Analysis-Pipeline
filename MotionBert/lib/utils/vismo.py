...
# def render_and_save(motion_input, save_path, keep_imgs=False, fps=30, color="#F96706#FB8D43#FDB381", with_conf=False, draw_face=False):
#     ensure_dir(os.path.dirname(save_path))
#     motion = copy.deepcopy(motion_input)
#     print(motion)
#     # Transpose if input is [T, J, C]
#     if motion.shape[0] not in [17, 6890]:  # Not already in [J, C, T]
#         motion = np.transpose(motion, (1, 2, 0))  # [T, J, C] -> [J, C, T]
    
#     if motion.shape[1] in [2, 3] and with_conf:
#         colors = hex2rgb(color)
#         motion_full = motion
#         motion_full[:, :2, :] = pixel2world_vis_motion(motion_full[:, :2, :], dim=2)
#         motion2video(motion_full, save_path=save_path, colors=colors, fps=fps, h=944, w=928)
#     elif motion.shape[1] == 3 and not with_conf:
#         motion_world = pixel2world_vis_motion(motion, dim=3)
#         motion2video_3d(motion_world, save_path=save_path, keep_imgs=keep_imgs, fps=fps)
#     elif motion.shape[1] == 6890:
#         motion2video_mesh(motion, save_path=save_path, keep_imgs=keep_imgs, fps=fps, draw_face=draw_face)
#     else:
#         raise ValueError("Unsupported motion shape or configuration")
def render_and_save(motion_input, save_path, keep_imgs=False, fps=25, color="#F96706#FB8D43#FDB381", with_conf=False, draw_face=False):
    ensure_dir(os.path.dirname(save_path))
    motion = copy.deepcopy(motion_input)
    if motion.shape[-1]==2 or motion.shape[-1]==3:
...
def get_img_from_fig(fig, dpi=100, target_size=(944, 928)):
    buf = io.BytesIO()
...
def joints2image(joints_position, colors, transparency=False, H=944, W=928, nr_joints=17, imtype=np.uint8, grayscale=False, bg_color=(255, 255, 255)):
    nr_joints = joints_position.shape[0]
    if nr_joints == 17:  # H36M joints
        limbSeq = [[0, 1], [1, 2], [2, 3], [0, 4], [4, 5], [5, 6], [0, 7], [7, 8], [8, 9], [8, 11], [8, 14], [9, 10], [11, 12], [12, 13], [14, 15], [15, 16]]
        L = rgb2rgba(colors[0]) if transparency else colors[0]
        M = rgb2rgba(colors[1]) if transparency else colors[1]
        R = rgb2rgba(colors[2]) if transparency else colors[2]
        ...
        raise ValueError("Only support 17 joints (H36M format)")
    ...
    for i in range(len(colors_joints)):
        if len(joints_position[i]) == 3:  # If confidence is present
            weight = joints_position[i][2]
            if weight == 0:
                continue
    ...
    for i in range(len(limbSeq)):
        ...
        point1 = joints_position[point1_index]
        point2 = joints_position[point2_index]
        if len(point1) == 3:  # Check confidence
            limb_weight = min(point1[2], point2[2])
            if limb_weight == 0:
                continue
        X = [point1[1], point2[1]]
    ...
    canvas = canvas.astype(imtype)
    return [canvas, canvas]  # Return same canvas for cropped/uncropped

def motion2video(motion, save_path, colors, h=512, w=512, bg_color=(255, 255, 255), transparency=False, motion_tgt=None, fps=30, save_frame=False, grayscale=False, show_progress=True, as_array=False):
...
def motion2video_3d(motion, save_path, fps=30, keep_imgs=False):
    ...
    for f in tqdm(range(vlen)):
        j3d = motion[:, :, f]
        fig = plt.figure(0, figsize=(9.44, 9.28), dpi=100)  # For 944x928
        ...
        frame_vis = get_img_from_fig(fig, dpi=100, target_size=(944, 928))
...
# def motion2video_mesh(motion, save_path, fps=30, keep_imgs = False, draw_face=True):
#     videowriter = imageio.get_writer(save_path, fps=fps)
#     vlen = motion.shape[-1]
...
#     print("smpl_faces sample:", smpl_faces[:5])
#     print("smpl_faces min/max:", smpl_faces.min(), smpl_faces.max())
#     frames = []
#     joint_pairs = [[0, 1], [1, 2], [2, 3], [0, 4], [4, 5], [5, 6], [0, 7], [7, 8], [8, 9], [8, 11], [8, 14], [9, 10], [11, 12], [12, 13], [14, 15], [15, 16]]
...
#     mid_y = (Y.max()+Y.min()) * 0.5
#     mid_z = (Z.max()+Z.min()) * 0.5
#     max_range = max_range * 1.5  # Increase range for visibility

#     print("X range:", mid_x - max_range, mid_x + max_range)
#     print("Y range:", mid_y - max_range, mid_y + max_range)
#     print("Z range:", mid_z - max_range, mid_z + max_range)
...
#                 limb = joint_pairs[i]
#                 xs, ys, zs = [np.array([j3d[limb[0], j], j3d[limb[1], j]]) for j in range(3)]
#                 ax.plot(-xs, -zs, -ys, c=[0,0,0], lw=3, marker='o', markerfacecolor='w', markersize=3, markeredgewidth=2)
#         elif draw_face:
#             ax.plot_trisurf(
#                 j3d[:, 0], j3d[:, 1], triangles=smpl_faces, Z=j3d[:, 2],
...
#         videowriter.append_data(frame_vis)
#         if keep_imgs:
#             plt.imsave(osp.join(save_name, f"frame_{f:04d}.png"), frame_vis)
#             frames.append(frame_vis)
#         plt.close()
